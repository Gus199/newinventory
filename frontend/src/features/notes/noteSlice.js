...
export const getNotes = createAsyncThunk(
  ...
  async (deviceId, thunkAPI) => {
    try {
      ...
      return await noteService.getNotes(deviceId, token)
    } catch (error) {
...
export const createNote = createAsyncThunk(
  'notes/create',
  async ({ noteText, deviceId }, thunkAPI) => {
    try {
      const token = thunkAPI.getState().auth.user.token
      return await noteService.createNote(noteText, deviceId, token)
    } catch (error) {
      return thunkAPI.rejectWithValue(extractErrorMessage(error))
    }
  }
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(getNotes.fulfilled, (state, action) => {
        // NOTE: even if there are no notes for the device we get an empty
